...
export interface UseThreadsReturn {
  ...
  addAssistantMessage: (threadId?: string) => Promise<ThreadMessage>
...
export function useThreads(): UseThreadsReturn {
  ...
  const loadThreads = useCallback(async () => {
    try {
      setIsLoading(true)
      const allThreads = await getAllThreads(false)
  ...
  const archiveCurrentThread = useCallback(async () => {
    ...
    setBranchState({})
    branchStateRef.current = {}
    log('Archived current thread:', threadId)
  }, [])
  ...
  const addAssistantMessage = useCallback(async (threadIdOverride?: string): Promise<ThreadMessage> => {
    ...
    // Parent is the last message (should be the user message we just added)
    const parentId = getLastMessageId()
