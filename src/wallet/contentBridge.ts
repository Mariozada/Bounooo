...
/**
 * Show sign overlay and actually sign the transaction via PHANTOM_SIGN
 */
function showSignOverlay(
  action: string,
  amount: number,
  transactionBase64?: string
): Promise<{ success: boolean; signature?: string; cancelled?: boolean; error?: string }> {
  return new Promise((resolve) => {
    document.getElementById('bouno-ov')?.remove();
    const ov = document.createElement('div');
    ov.id = 'bouno-ov';
    ov.style.cssText = 'position:fixed;inset:0;z-index:2147483647;background:rgba(0,0,0,.9);display:flex;align-items:center;justify-content:center;font-family:system-ui';
    ov.innerHTML = '<div style="background:linear-gradient(145deg,#1a1a2e,#16213e);border-radius:20px;padding:32px;max-width:360px;width:90%;text-align:center;border:1px solid rgba(255,255,255,.1)">'
      + '<div style="width:72px;height:72px;margin:0 auto 20px;background:linear-gradient(135deg,#4ade80,#22d3ee);border-radius:18px;display:flex;align-items:center;justify-content:center;font-size:36px">✍️</div>'
      + '<h2 style="color:#fff;margin:0 0 8px;font-size:22px">Confirm Transaction</h2>'
      + '<p style="color:#94a3b8;margin:0 0 12px;font-size:14px">' + action + '</p>'
      + '<p style="color:#4ade80;margin:0 0 24px;font-size:28px;font-weight:700">' + amount + ' SOL</p>'
      + '<button id="ba" style="width:100%;padding:14px;font-size:16px;font-weight:600;border:none;border-radius:12px;cursor:pointer;background:linear-gradient(135deg,#4ade80,#22d3ee);color:#1a1a2e;margin-bottom:12px">Approve</button>'
      + '<button id="bx" style="width:100%;padding:12px;font-size:14px;background:transparent;border:1px solid rgba(255,255,255,.2);border-radius:12px;color:#94a3b8;cursor:pointer">Cancel</button>'
      + '<p id="be" style="color:#f87171;margin:16px 0 0;font-size:13px;display:none;padding:10px;background:rgba(248,113,113,.1);border-radius:8px"></p>'
      + '</div>';
    document.body.appendChild(ov);
    const done = () => ov.remove();
    const ba = document.getElementById('ba') as HTMLButtonElement | null;
    const bx = document.getElementById('bx');
    const be = document.getElementById('be');
    ...
    if (ba) {
      ba.onclick = async () => {
        if (!transactionBase64) {
          // No transaction to sign — this shouldn't happen for real purchases
          done();
          resolve({ success: false, error: 'No transaction provided' });
          return;
        }

        ba.textContent = 'Signing...';
        ba.disabled = true;

        try {
          // Send to background which will execute in MAIN world via PHANTOM_SIGN
          const result = await chrome.runtime.sendMessage({
            type: 'PHANTOM_SIGN',
            transactionBase64,
            rpcUrl: 'https://api.devnet.solana.com',
          }) as { success: boolean; signature?: string; error?: string; cancelled?: boolean };

          if (result.success && result.signature) {
            // Show success briefly
            ba.textContent = 'Success!';
            ba.style.background = '#4ade80';
            setTimeout(() => {
              done();
            ...
            if (be) { be.textContent = result.error || 'Transaction failed'; be.style.display = 'block'; }
            ba.textContent = 'Try Again';
            ba.disabled = false;
          }
        } catch (e) {
          if (be) { be.textContent = (e as Error).message; be.style.display = 'block'; }
  ...
  (async () => {
    try {
      if (msg.type === 'WALLET_BRIDGE_CHECK') {
        ...
        // Real signing flow — show overlay, then sign via PHANTOM_SIGN in MAIN world
        const result = await showSignOverlay(
          msg.action || 'Purchase',
          msg.amount || 0,
          msg.transactionBase64,
        );
        send(result);
      }
    } catch (e) {
